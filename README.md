...
Looking at the results we may judge if our simulation reaches a dead end (for example deltaT could approach unsustainable values, e.g. endTime = 10, deltaT = 1e-8 and a duration per timeStep of 30s would take almost a year) or how it behaves in general. In this example, these are the results from the also shared Francis turbine, we obviously increased maxCo and at the same time nCorrectors. We decreased nCorrectors after 1.01*timeStep_maxCo has passed (this is decrease_nCorrectors_factor). 
Looking at the achieved_deltaT we clearly find a linear increase after the last change of maxCo. This is a transient simulation with constant maxCo, so an increasing deltaT is a good sign (initial transients are still fading away). 
